...
class SoHocAPIView(APIView):
    def get(self, request):
        ...
        main_number_data = MainNumber.objects.filter(code=so_chu_dao).first()
        full_name = request.GET.get('full_name').upper()
        html = loader.render_to_string('invoice.html', {
            ...
            'so_no_nghiep': ', '.join(so_no_nghiep),
            'so_thieu': '',
            ...
            'main_number_data': main_number_data
        })
        options = {
        'page-size': 'Letter',
        'margin-top': '0.7in',
        ...
        'margin-left': '0.75in',
        'encoding': "UTF-8",
        'header-left': full_name,
        'header-right': 'NGÀY SINH ' + birth_day,
        'header-spacing': '5',
        'footer-font-size':'10',
        'footer-center': '[page]',
        'footer-spacing': '2',
